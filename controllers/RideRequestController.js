...
const createRideRequest = async (req, res) => {
    ...
    const rideRequestHandle = new RideRequest(req.body);
    rideRequestHandle
        .save()
        .then((data) => {
            res.status(200).send({
...
const putRideRequest = async (req, res) => {
    ...
    RideRequest.findByIdAndUpdate(req.params['rideRequestId'], { $set: req.body }, {upsert:true, useFindAndModify: false},
        (err, updatedData) => {
...
const getRideRequests = async (req, res) => {
    RideRequest.find({seatsRequired: {$gte:0}}, null, null,
        (err, updatedData) => {
